            ...
            if (sysnum_combo5.Items.Count == 11 && sysnum_combo4.Items.Count == 11 && sysnum_combopc.Items.Count == 11)
            {
                ...
                switch (tabControl1.SelectedTab.Name)
                {
                    ...
                    case "tabPage2": // در صورت انتخاب تب دوم
                        ...
                        break;
                    case "tabPage3": // در صورت انتخاب تب سوم
                        if (sysnum_combopc.SelectedItem.ToString() != "انتخاب کنید") // اگر آیتم انتخاب شده متفاوت از "انتخاب کنید" باشد
                        {
                            cost_grppc.Enabled = true; // فعال کردن گروه هزینه
                        }
                        else
                        {
                            cost_grppc.Enabled = false; // غیرفعال کردن گروه هزینه
                        }
                        break;
                }
            }
